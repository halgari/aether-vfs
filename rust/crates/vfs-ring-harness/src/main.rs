...
//!
//! **The Windows-only ring harness.** It maps a *named*, page-file-backed
//! section (`SharedMapping::open`), which has identity only inside one Windows
//! or Wine session — nothing a native Linux process could open — so this
//! binary is Windows-only by construction and `vfs-win` is only in the
//! `cfg(windows)` dependency table. Gated like its `ring-file-*` siblings so
//! `cargo build -p vfs-ring-harness` is green on Linux with no `--bin` filter;
//! the cross-boundary pair is `ring-file-client` / `ring-file-server`.
