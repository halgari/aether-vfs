...
async function main() {
  ...
  const cdn = await providerWorker({
    module: require.resolve('./pretend-cdn-provider.cjs'),
  ...
  const wrong = registerProvider(require('./pretend-cdn-provider.cjs')({ latencyMs: 0 }));
  ...
  try {
    ...
    for (const line of e.message.replace(/\. /g, '.\n').split('\n')) {
